...
def crf_log_likelihood(inputs,
    ...
    if transition_params is None:
        transition_params = vs.get_variable("transitions",
                                            [num_tags, num_tags])
...
class CrfDecodeForwardRnnCell(rnn_cell.RNNCell):
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        backpointers = math_ops.cast(backpointers,
                                     dtype=dtypes.int32)  # [B, O]
...
def crf_decode(potentials, transition_params, sequence_length):
    ...
    return utils.smart_cond(
        pred=math_ops.equal(
            tensor_shape.dimension_value(potentials.shape[1]) or
