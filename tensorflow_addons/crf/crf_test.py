...
from tensorflow_addons.crf import crf


class CrfTest(tf.test.TestCase):
    def calculate_sequence_score(self, inputs, transition_params, tag_indices,
                                 sequence_lengths):
    ...
    def test_crf_sequence_score(self):
        ...
        for sequence_lengths, inputs, tag_indices in zip(sequence_lengths_list,
            ...
            with self.cached_session() as sess:
                sequence_score = crf.crf_sequence_score(
                    ...
                    sequence_lengths=array_ops.expand_dims(sequence_lengths,
                                                           0),
        ...
        for sequence_lengths, inputs, tag_bitmap in zip(
            ...
            with self.cached_session() as sess:
                ...
                expected_sequence_scores = [
                    self.calculate_sequence_score(inputs, transition_params,
                                                  indices,
                                                  sequence_lengths)
    ...
    def test_crf_binary_score(self):
    ...
    def test_crf_log_norm_zero_seq_length(self):
        """
    ...
    def test_crf_log_likelihood(self):
        inputs = np.array(
            [[4, 5, -3], [3, -1, 3], [-1, 2, 1], [0, 0, 0]], dtype=np.float32)
        transition_params = np.array(
            [[-3, 5, -2], [3, 4, 1], [1, 2, 1]], dtype=np.float32)
    ...
    def test_crf_decode(self):
        ...
        for sequence_lengths, inputs, tag_indices in zip(sequence_lengths_list,
            ...
            with self.cached_session() as sess:
                ...
                expected_max_sequence_index = np.argmax(tf_all_sequence_scores)
                expected_max_sequence = all_sequences[
                    expected_max_sequence_index]
                expected_max_score = tf_all_sequence_scores[
                    expected_max_sequence_index]
                ...
                actual_max_sequence = array_ops.squeeze(actual_max_sequence,
                                                        [0])
    ...
    def test_crf_decode_zero_seq_length(self):
        """
        ...
        with self.cached_session() as sess:
            ...
            sequence_lengths = constant_op.constant(np.zeros([2],
                                                             dtype=np.int32))
            tags, scores = crf.crf_decode(inputs, transition_params,
                                          sequence_lengths)
