...
class CRF(tf.keras.layers.Layer):
    ...
    def mask_to_sequence_length(self, mask):
        ...
        sequence_length = tf.keras.backend.cast(
            tf.keras.backend.sum(tf.dtypes.cast(mask, tf.int64), 1), tf.int64)
        return sequence_length
    ...
    def _compute_mask_left_boundary(mask):
        ...
        left_boundary = tf.keras.backend.greater(tf.dtypes.cast(mask, tf.int64), tf.dtypes.cast(right_shifted_mask, tf.int64))
